...
abstract class PublishDateExtractor extends Extractor[Date] {
  ...
  * <p>This will be called by the {@link com.jimplush.goose.ContentExtractor#extractContent(String)} method and will be passed to {@link com.jimplush.goose.Article#setPublishDate(java.sql.Date)}</p>
  *
...
object PublishDateExtractor extends Logging {
  ...
  /**
    ...
    */
  def minDate(lhs: java.sql.Date, rhs: java.sql.Date): java.sql.Date = {
  ...
  /**
    ...
    */
  def safeParseISO8601Date(txt: String): Option[java.sql.Date] = {
    if (txt == null || txt.isEmpty)
      return None

    try {
      Option(new Date(datatypeFactory.newXMLGregorianCalendar(txt).toGregorianCalendar.getTime.getTime))
    } catch {
      case ex: Exception =>
