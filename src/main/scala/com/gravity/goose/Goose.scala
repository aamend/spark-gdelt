...
class Goose(config: Configuration = new Configuration) {
  ...
  * @param url The url that you want to extract
  */
  ...
  def initializeEnvironment() {
    ...
    if (!f.canWrite)
      throw new Exception(config.localStoragePath + " directory is not writable, you need to set this for image processing downloads")
